...
> 엘리스 ML API 크레딧은 기관 생성·승인 절차를 거쳐 지급되므로, 수령 이전에는 OpenAI API(생성 `gpt-4o-mini`, judge `gpt-4o`)로 파이프라인과 Part C 실험 1번부터 7번까지를 구축했습니다. 이하 문서에서는 이 시기를 "개발기"로 표기합니다. 크레딧 수령 후 아래의 엘리스 공식 모델로 전환해 최종 baseline과 실험 8번부터 10번까지를 측정했습니다. 임베딩 모델을 동일 계열(`text-embedding-3-small`)로 선택해 둔 덕분에 전환 시 재인덱싱 없이 기존 인덱스를 재사용했으며, 전환 전후의 지표 변화는 원인까지 분석했습니다(실험 8). 아래 표는 최종 구성 기준입니다.
...
| MRR | 0.873 | 0.873 | |
| Citation Precision | 0.703 | 0.746 | 실험 1과 동일한 +0.04에서 +0.06 사이 패턴이 세 번째로 재현 |
...
- **Claude Sonnet 5는 정당한 대안입니다.** GPT-5.6 Sol과 Faithfulness·False Refusal이 같고 Correctness·Citation Precision은 조금 앞서지만(각 +0.017, +0.012), 그 폭은 판정 분산(±0.5 등급, 문항 1~2개) 범위 안이라 품질이 유의미하게 갈린다고 보기 어렵습니다. 품질이 대등하니 크레딧 한도 안에서 반복 실험에 유리한 경량 모델(GPT-5.6 Sol)을 택했습니다. 비용 제약에 따른 trade-off이고, Claude Sonnet 5는 품질 우선 환경에서 바로 교체 가능한 후보로 남겨 둡니다.
